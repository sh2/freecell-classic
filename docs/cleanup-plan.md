...
- [ ] **A1** `README.md` の「ドキュメント」セクションのリンクを実体のある
  `docs/archive/` 配下へ修正する(または 4.2 で `ui-state-matrix.md` を
  `docs/` へ戻す場合はそのリンクに合わせる)。
- [ ] **A2** `view.js: syncControls()` のコメントのパス参照を修正する。
- [ ] **A3** `docs/archive/ui-state-matrix.md` を「生きた仕様」として `docs/` 直下へ戻す(既定案)。
...
- [ ] 変更した Markdown ファイルの markdownlint 診断を確認し、指摘を解消する。
