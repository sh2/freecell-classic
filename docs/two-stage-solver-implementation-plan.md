...
| 完全性 | 実装・テストで段階的に保証する |
| 上限到達時 | `node-limit` / `time-limit` |
| 主な評価指標 | 高速モードからの追加解決数、探索漏れの有無 |

完全性の検証が完了するまでは、UI・ログ・文書上の名称を「安全探索」とし、
「完全探索」「解なし証明」という表現は使用しない。
...
| `unsolvable` | 完全性を保証できる場合だけ使用する予約値 |
...
安全モードでは、最初に「残りカード数」だけを確実な下限として使用し、既存の
「可動末尾より下のカード数」は順序評価へ移す案を基準とする。性能低下を測定した上で、
証明可能な下限だけを追加する。
