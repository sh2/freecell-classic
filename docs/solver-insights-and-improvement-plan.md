...
#### 3.4.1 フェーズD実施結果（2026-08-19）

最終状態（低ランク露出加点 +6000、`tailStart` 重み1.1、fast上限200万ノード、
safe上限1,000万ノード、safe2 リトライ付き）で、全32,000ゲームを現行ロジックで再計測した
（`scripts/benchmark/run.js --all --force`、`docs/benchmark/data/batch-*.json`）。

#### 全32,000ゲームの再計測結果（現行ロジック + safe2）

| 指標 | 値 |
| --- | ---: |
| 解決 | **31,999 / 32,000** |
| `node-limit` | **1件**（`#11982`、解けないことで有名） |
| `time-limit` | 0件 |
| `unsolvable` | 0件 |
| 総ノード | 432,907,213 |
| 総時間 | 714,964 ms |
| フォールバック | 22件 |
| fast 単体で解決 | 31,978件（99.93%） |

- 旧ロジックの履歴値（31,974/32,000、`node-limit` 26件）から、解決率が改善し
  `node-limit` が 26件 → 1件 に減少した。これは低ランク露出加点・`tailStart` 1.1・
  対称性削減・safe2 リトライの効果である。
- **`#10353` / `#13331` は safe2 リトライで解決可能になった**（詳細は §3.4.3）。
- fast 単体ノード数分布（解決ゲームのみ）: p50=2,027 / p90=19,763 / p99=152,775 /
  p99.9=670,966 / max=1,980,517。
- safe単一モードは fast-safe より解決率を改善せず、通常サンプルの総ノード数・総時間を
  大きく増加させるため、単一safeへの統合は採用しない。

#### fast 上限の感度分析

`scripts/benchmark/fast-cap-sensitivity.js` で、fast 上限を変えたときのフォールバック数を
シミュレーションした（各ゲームの fast 単体ノード数から推定）。

| fast 上限 | fast 解決 | safe フォールバック |
| ---: | ---: | ---: |
| 250,000 | 31,817 | 183 |
| 500,000 | 31,929 | 71 |
| 750,000 | 31,952 | 48 |
...
- **fast で解けるが safe では解けないゲームが存在する**（`#17978`、fast=1,271,789）。
  このゲームは fast では解決するが、safe 単体では `node-limit` になる。したがって、
  fast 上限を 130万 未満に下げると `#17978` が safe に落ちて解けなくなり、解決率が下がる。
- 上限 130万 で safe に落ちる13ゲームはすべて safe で解決するため、**上限 130万 以上なら
  解決率は維持**できる（`node-limit` は fast-safe でも解けない既知の3件のみ）。
- fast で node-limit になる22ゲームは、すべて fast で 200万 ノードを消費してから safe に
  落ちる。上限を下げるとこの fast 消費が減り、難関ゲームの応答時間が短縮される。
- 一方、fast で解けるが上限を超えるゲームが safe に落ちると、safe の方が遅い場合がある
  （例: `#14752` は fast=1,980,517 ノードだが safe=2,806,707 ノード）。

#### 3.4.2 判定と今後の方針

- **二段階 fast-safe を維持する。** 通常ゲームではfastの低コストを利用し、難関だけをsafeへ
  送る現在の構成が、解決率と応答時間のバランスに優れる。
- **fastを単一の既定モードへ置き換える変更は行わない。** 境界例で解決率を下げるため、
  UI/APIの既定戦略は `fast-safe` のままとする。
- **fast 上限の変更は「0.数%の解を失う」ことにつながるため、慎重に判断する。**
  fast-safe 構成では fast 単体の解決率は最終結果に影響しないが、fast で解けるが safe では
  解けないゲーム（`#17978`）が存在するため、上限を下げすぎると解決率が下がる。
  解決率を維持するには上限を 130万 以上に保つ必要がある。
- **fast 上限を 200万 → 150万 に下げることを推奨。** 解決率は維持でき（`node-limit` 3件のみ）、
  fast で node-limit になる22ゲームの応答時間を短縮できる。ただし、fast で解けるが
  150万超の6ゲームが safe に落ちるため、safe の方が遅い場合は応答時間が長くなる。
  このトレードオフは、実際に上限を変更して全32,000ゲームを再計測して確認する必要がある。
- 残る改善課題は、safe段の探索効率改善、および上限値を変更した場合の全32,000ゲーム再計測である。

#### 3.4.3 safe2 再試行の追加（2026-08-19）

未解決3件（`#10353` / `#11982` / `#13331`）のうち、解けないことで有名な `#11982` を除く
2件を解けるようにするため、**safe が上限で打ち切られたときに、より強いヒューリスティックで
再試行する「safe2」段階**を追加した。

- **背景**: `#10353` は safe で 58.9M ノード、`#13331` は 17.4M ノード必要で、現在の
  safe 上限（10M）では不足していた。`tailStart` 重みを強くすると、これらのゲームは
  少ないノードで解ける（`#10353` は w=1.5 で 935 ノード、`#13331` は 1.33M ノード）。
- **実装**: `SOLVER_PROFILES.safeRetry`（`maxNodes=2M`、`tailStartWeight=1.5`）を追加し、
  `solveWithFallback` で safe が失敗した場合に safe2 として再試行する。`options.safeRetry`
  で無効化できる。
- **効果**:
  - `#10353` / `#13331` が safe2 で解決可能になった。
  - 既に safe で解けるゲーム（例: `#24106`）は safe2 に到達しないため、挙動は不変。
  - 本当に解けない `#11982` は safe2 でも失敗するが、safe2 の上限が 2M のため、
    追加コストは 2M ノード（約2秒）に留まる。
- **検証**: 単体 141 件・E2E 63 件すべて成功。全32,000ゲームの再計測で
